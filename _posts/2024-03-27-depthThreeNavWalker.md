---
title: Creating a 3 Depth Nav Walker
date: 2024-03-27 12:00:00 -600
categories: [wordpress, bootstrap, understrap]
...
if ( ! class_exists( 'Understrap_WP_Bootstrap_Navwalker' ) ) {
	...
	class Understrap_WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
        ...
        // Add .dropdown-menu class to the dropdown's <ul> element, and an additional class for depth
        $classes = array( 'dropdown-menu' );
			if ($depth === 1) { // Check if this is a second level menu
				$classes[] = 'second-level-menu'; // This is for styling the second level dropdown
			} elseif ($depth > 1) {
				$classes[] = 'third-level-menu'; // This is for styling the third level dropdown
			}
        $class_names = join( ' ', apply_filters( 'nav_menu_submenu_css_class', $classes, $args, $depth ) );
        $class_names = $class_names ? ' class="' . esc_attr( $class_names ) . '"' : '';

        // Adjusted to add classes based on depth
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( isset( $args->has_children ) && $args->has_children && 1 !== $args->depth ) {
				...
				$atts['id']             = 'menu-item-dropdown-' . $item->ID . ' has-children-depth-' . $item->depth;
			} else {
				...
				if ( $depth > 0 ) {
					...
					$atts['id'] = 'no-children depth-' . $depth;
					$atts['class'] = 'nav-link';
				}
			}
...
document.addEventListener("DOMContentLoaded", function () {
  // Query Selector for the dropdown items
  var dropdowns = document.querySelectorAll(".dropdown-toggle");

  dropdowns.forEach(function (dropdown) {
    dropdown.addEventListener("click", function (e) {
      var nextMenu = this.nextElementSibling;
      if (nextMenu && nextMenu.classList.contains("second-level-menu")) {
        e.preventDefault(); // Prevent navigating to the link href
        nextMenu.classList.toggle("show"); // Toggle the display of the submenu
      }
    });
  });
